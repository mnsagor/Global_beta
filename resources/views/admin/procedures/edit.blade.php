    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.procedures.update", [$procedure->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $procedure->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('modality') ? 'is-invalid' : '' }}" name="modality_id" id="modality_id" required>
                    @foreach($modalities as $id => $modality)
                        <option value="{{ $id }}" {{ ($procedure->modality ? $procedure->modality->id : old('modality_id')) == $id ? 'selected' : '' }}>{{ $modality }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('procedure_type') ? 'is-invalid' : '' }}" name="procedure_type_id" id="procedure_type_id" required>
                    @foreach($procedure_types as $id => $procedure_type)
                        <option value="{{ $id }}" {{ ($procedure->procedure_type ? $procedure->procedure_type->id : old('procedure_type_id')) == $id ? 'selected' : '' }}>{{ $procedure_type }}</option>
                    @endforeach
                </select>
